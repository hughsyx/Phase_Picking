...
# read filename 
	...
	if len(picksn)>2:
		...
		for i in range(0,len(picksn)-1):
			amp_tmp[i] = np.mean(abs(trn.slice(picksn[i],picksn[i+1]).data))
		amp_tmp[-1] = np.mean(abs(trn.slice(picksn[-1],endtime).data))
	...
	if len(pickse)>2:
                ...
                for i in range(0,len(pickse)-1):
                        amp_tmp[i] = np.mean(abs(tre.slice(pickse[i],pickse[i+1]).data))
                amp_tmp[-1] = np.mean(abs(tre.slice(pickse[-1],endtime).data))
                sort_indx = amp_tmp.argsort()[::-1]
                remove_indx = sort_indx[2:]
                for idx in remove_indx:
...
def runPickers(detect_file):
	...
	if len(pickp) >0:
		...
		if len(picks_new)>0:
			...
			# new file name
			dir_out = '/Volumes/Untitled/Wenchuan_Mostafa/PICK/'
		else:
			dir_out = '/Volumes/Untitled/Wenchuan_Mostafa/PICK_FAIL/'
...
detect_file = '20080731024012.LUYA.BHZ'
